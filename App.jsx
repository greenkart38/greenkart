...
const BREVO_API_KEY = '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'
const BREVO_SENDER = 'GreenKart'
...
// Adulte prend les créneaux pairs (0,2,4...), enfant les impairs (1,3,5...)
// en sautant les pauses
function generateInterleavedSlots(startTime, nbAdulte, nbEnfant, allSlots) {
  ...
  const enfantSlots = []
  let idx = startIdx
  let turn = 0 // 0=adulte, 1=enfant

  const totalNeeded = nbAdulte + nbEnfant
  let placed = 0

  while (placed < totalNeeded && idx < allSlots.length) {
    // Sauter les pauses
    ...
    if (turn === 0 && adulteSlots.length < nbAdulte) {
      adulteSlots.push(slot)
      placed++
      turn = nbEnfant > 0 ? 1 : 0
    } else if (turn === 1 && enfantSlots.length < nbEnfant) {
      // Vérifier limite 18h pour enfant
      if (slot < `${String(ENFANT_MAX_HOUR).padStart(2,'0')}:00`) {
        enfantSlots.push(slot)
        placed++
      }
      turn = nbAdulte > 0 && adulteSlots.length < nbAdulte ? 0 : 1
    } else {
      // Compléter ce qui manque
      if (adulteSlots.length < nbAdulte) { adulteSlots.push(slot); placed++ }
      else if (enfantSlots.length < nbEnfant && slot < `${String(ENFANT_MAX_HOUR).padStart(2,'0')}:00`) { enfantSlots.push(slot); placed++ }
      else break
    }
    idx++
  }
...
async function sendSMS(phone, message) {
  try {
    const resp = await fetch('https://api.brevo.com/v3/transactionalSMS/sms', {
      ...
      body: JSON.stringify({ sender: BREVO_SENDER, recipient: phone.replace(/\s/g,''), content: message, type: 'transactional' })
    })
...
function buildSMS(cfg, name, date, time, session, participants, depositLink) {
  return (cfg.sms||'').replace('{nom}', name).replace('{karting}', cfg.name+' '+cfg.city).replace('{date}', new Date(date+'T12:00:00').toLocaleDateString('fr-FR')).replace('{heure}', time).replace('{session}', session?.label||'').replace('{participants}', participants).replace('{acompte}', session?.deposit||'').replace('{lien}', depositLink)
}
...
export default function App() {
  ...
  async function saveReservation(items) {
    ...
    for (const item of items) {
      ...
      savedIds.push({ id, item })
    }

    // SMS — envoyer 1 seul SMS récapitulatif pour toutes les réservations
    if (items.length > 0) {
      const first = items[0]
      const session = cfg.sessions.find(x=>x.id===first.session)||cfg.sessions[0]
      const depositLink = `https://pay.greenkart.fr/${savedIds[0].id}`
      const totalSessions = items.length
      const msg = buildSMS(cfg, first.name, first.dateStr, first.time, session, first.participants, depositLink)
        + (totalSessions > 1 ? `\n${totalSessions} créneaux réservés.` : '')
      const smsRes = await sendSMS(first.phone, msg)
      if (smsRes.ok) showToast(`✓ ${totalSessions > 1 ? totalSessions+' réservations créées' : 'Réservation confirmée'} · SMS envoyé 📱`)
      else showToast(`✓ Réservation${totalSessions>1?'s':''} créée${totalSessions>1?'s':''} · SMS échoué (${smsRes.error||JSON.stringify(smsRes.data)})`)
    }
  ...
  async function resenSMS(res) {
    ...
    showToast(r.ok ? `📱 SMS renvoyé → ${res.phone}` : `❌ SMS échoué`)
  }
  ...
  return (
    <div style={S.app}>
      ...
      <div style={{display:'flex',gap:10,padding:'6px 0',borderBottom:'1px solid #e5e7eb',flexWrap:'wrap',fontSize:11,color:'#6b7280',marginTop:6}}>
        ...
        <span>⏸ Heures piles = pause · 🟢 Adulte · 🟡 Enfant (14h→18h)</span>
      </div>
      ...
      {modal && (
        <div style={S.overlay} onClick={()=>setModal(null)}>
          <div onClick={e=>e.stopPropagation()}>
            ...
            {modal.type==='detail' && <DetailModal cfg={cfg} res={reservations.find(r=>r.id===modal.res.id)||modal.res} onToggleArrived={toggleArrived} onToggleAcompte={toggleAcompte} onDelete={deleteRes} onUpdate={updateRes} onReSMS={resenSMS} onClose={()=>setModal(null)}/>}
                      ...
                      {sr.map((r,ri)=>{
                        ...
                        return (
                          <div key={r.id} onClick={e=>{e.stopPropagation();setModal({type:'detail',res:r})}}
                            style={{position:'absolute',top:2,bottom:2,left:`${ri*w}%`,width:`calc(${w}% - 3px)`,background:r.arrived?'#D1FAE5':s?s.bg:'#f3f4f6',border:`1px solid ${r.arrived?'#1D9E75':s?s.color:'#e5e7eb'}`,borderRadius:3,padding:'0 4px',fontSize:9,fontWeight:600,color:r.arrived?'#065F46':s?s.color:'#374151',overflow:'hidden',whiteSpace:'nowrap',textOverflow:'ellipsis',display:'flex',alignItems:'center',gap:2,cursor:'pointer'}}>
        ...
        return (
          <div key={time} style={{marginBottom:8}}>
            ...
            {slotRes.map(r=>{
              ...
              const hasAcompte=r.acompte_paid
              return (
                <div key={r.id} style={{display:'flex',alignItems:'center',gap:8,padding:'9px 11px',border:`1.5px solid ${isArrived?'#1D9E75':'#fca5a5'}`,borderRadius:9,marginBottom:5,marginLeft:16,background:isArrived?'#f0faf5':'#fff8f8',transition:'all 0.2s'}}>
                  {/* Bouton arrivée */}
                  <div onClick={()=>toggleArrived(r.id)}
                    style={{width:26,height:26,borderRadius:'50%',border:`2px solid ${isArrived?'#1D9E75':'#fca5a5'}`,cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center',fontSize:13,background:isArrived?'#1D9E75':'#fff',color:isArrived?'#fff':'#fca5a5',flexShrink:0,transition:'all 0.2s'}}>
                    {isArrived?'✓':'!'}
                  </div>
                  <div style={{flex:1,minWidth:0}}>
                    <div style={{fontSize:13,fontWeight:600,textDecoration:isArrived?'line-through':'none',color:isArrived?'#9ca3af':'#111'}}>{r.name}</div>
                  ...
                  <div style={{display:'flex',flexDirection:'column',alignItems:'flex-end',gap:5}}>
                    <button onClick={()=>toggleAcompte(r.id)}
                      style={{fontSize:11,padding:'3px 9px',borderRadius:20,border:`1.5px solid ${hasAcompte?'#1D9E75':'#e5e7eb'}`,cursor:'pointer',background:hasAcompte?'#1D9E75':'#fff',color:hasAcompte?'#fff':'#9ca3af',fontWeight:hasAcompte?600:400,transition:'all 0.2s'}}>
                      {hasAcompte?'✓ Acompte reçu':`Acompte ${s?.deposit||'?'}€`}
                    </button>
...
function NewResModal({cfg,dateStr,time,reservations,onSave,onClose}) {
  ...
  // Créneaux intercalés adulte/enfant
  ...
  return (
    <div style={S.modal}>
      ...
      {/* Date et heure modifiables en haut */}
      ...
      {previewSlots.length>0&&(
        <div style={{background:'#f9fafb',borderRadius:9,padding:'10px 12px',marginBottom:14,border:'1px solid #e5e7eb'}}>
          <div style={{fontSize:10,fontWeight:700,color:'#9ca3af',marginBottom:8,textTransform:'uppercase',letterSpacing:'0.5px'}}>📅 Aperçu planning</div>
          {previewSlots.map(t=>{
      ...
      <div style={{fontSize:11,color:'#9ca3af',marginBottom:10,display:'flex',alignItems:'center',gap:5}}>
        📱 Un SMS de confirmation sera envoyé automatiquement
      </div>
...
// ─── DETAIL MODAL ─────────────────────────────────────────────────────────────
function DetailModal({cfg,res,onToggleArrived,onToggleAcompte,onDelete,onUpdate,onReSMS,onClose}) {
  ...
  return (
    <div style={S.modal}>
      ...
      {/* Statuts avec couleurs dynamiques */}
      <div style={{display:'flex',gap:6,marginBottom:13,flexWrap:'wrap'}}>
        <div onClick={()=>onToggleArrived(res.id)} style={{fontSize:12,padding:'6px 12px',borderRadius:20,background:res.arrived?'#1D9E75':'#FEE2E2',color:res.arrived?'#fff':'#dc2626',cursor:'pointer',fontWeight:600,transition:'all 0.2s',border:`1.5px solid ${res.arrived?'#1D9E75':'#fca5a5'}`}}>
          {res.arrived?'✓ Arrivé':'⚠️ En attente'}
        </div>
        <div onClick={()=>onToggleAcompte(res.id)} style={{fontSize:12,padding:'6px 12px',borderRadius:20,background:res.acompte_paid?'#1D9E75':'#f3f4f6',color:res.acompte_paid?'#fff':'#6b7280',cursor:'pointer',fontWeight:600,transition:'all 0.2s',border:`1.5px solid ${res.acompte_paid?'#1D9E75':'#e5e7eb'}`}}>
          {res.acompte_paid?'💳 Acompte reçu':'💳 Acompte en attente'}
        </div>
      </div>
      ...
      {editing?(
        ...
        <>
          {[['Session',s?<span style={S.badge(s.bg,s.color)}>{s.label}</span>:'—'],['Kart',res.kart_type],['Participants',res.participants],['Total',s?s.price*res.participants+'€':'—'],['Acompte à régler',s?s.deposit+'€':'—'],['Téléphone',res.phone],['Email',res.email||'—']].map(([l,v])=>(
      ...
      <div style={{display:'flex',gap:6,justifyContent:'flex-end',flexWrap:'wrap'}}>
        ...
        <button style={S.btn('primary')} onClick={()=>onReSMS(res)}>📱 Renvoyer SMS</button>
      </div>
    </div>
  )
}
...
function EventsPage({cfg,reservations,saveReservation}) {
  ...
  return (
    <div style={{paddingTop:14,maxWidth:580}}>
      ...
      <div style={{fontSize:12,color:'#9ca3af',marginBottom:14}}>Créneaux toutes les 15min · Pauses auto · Inscrit dans le planning + SMS envoyé</div>
      ...
      <button style={{...S.btn('primary'),padding:'10px 24px',fontSize:13,width:'100%'}} onClick={handleCreate}>
        🏁 Créer + inscrire dans le planning + SMS ({eventSlots.length} créneaux)
      </button>
    </div>
  )
}
      ...
      {section==='sms'&&(
        <div>
          <div style={{background:'#E1F5EE',border:'1px solid #1D9E75',borderRadius:8,padding:'10px 12px',marginBottom:12,fontSize:12,color:'#0F6E56',lineHeight:1.6}}>
            ✅ <b>SMS Brevo activé</b> — clé API intégrée.<br/>
            Les SMS sont envoyés automatiquement à chaque confirmation de réservation.
          </div>
          <div style={S.field}>
